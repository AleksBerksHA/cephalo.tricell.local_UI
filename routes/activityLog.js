        ...
        async function sqlQuery()
        {
            ...
            if(request.session.loggedin)
            {
                ...
                response.write(pug_loggedinmenu({
                    ...
                    securityaccesslevel: request.session.securityaccesslevel || request.cookies.securityaccesslevel,
           ...
           // 1. Skapa säkra variabler (om cookies saknas används 20 och ID)
            const sortAmount = request.cookies.sortAmount || 20; 
            const sortType = request.cookies.sortType || "ID";

            let result;

            // 2. Kör frågan med de säkra variablerna och hakparenteser [ ]
            if (sortType === "ID") {
            ...
            {   
                ...
                if(canEdit(request))
                {
                    htmlOutput += "<div class=\"table-body-cell\"><a href=\""+ config.webaddress+"/api/activitylog/" + str_id + "\" style=\"color:red;text-decoration:none;\">D</a></div>\n"; // Gör till knapp
                }
                htmlOutput += "</div>\n";
            }  
    ...
    async function deleteSQLquery()
    {
        ...
        if(canEdit(request))  // check permissions first
        {
            ...
            if(result.length > 0)
            {
                ...
                response.write("<a href=\""+ config.webaddress+"/api/activitylog\" style=\"color:#336699;\">Back to Activity Log</a>");
            }
            else
            {
