...
router.get('/deletevirusimage/:virusId/:imageNumber', function(request, response) {
    ...
    async function deleteImage() {
        ...
        response.write(`<a href="${config.webaddress}/api/editvirus/${virusId}" style="...">Back to Edit Virus</a>`);        response.write(htmlInfoStop);
    ...
    if(request.session.loggedin && (request.session.securityAccessLevel == "B" || request.session.securityAccessLevel == "A")) {
        ...
        form.parse(request, function(err, fields, files) {
            async function handleFile() {
                ...
                response.write(`<a href="${config.webaddress}/api/editvirus/${virusId}" style="display:inline-block; margin-top:20px; padding:10px 20px; background-color:#007BFF; color:#fff; text-decoration:none; border-radius:5px;">Back to Edit Virus</a>`);
