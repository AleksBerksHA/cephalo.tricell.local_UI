...
// Parser för att kunna skapa nya objekt
...
router.get('/', (request, response) => {
    ...
    async function sqlQuery() {
        ...
        result.forEach(row => {
            ...
            // Om den är arkiverad (men vi är nivå A)
    ...
    async function sqlQuery() {
        try {
            ...
            if (request.session && request.session.loggedin) {
                ...
                response.write(htmlHeader); 
                // skriv ut den inloggade menyn via Pug
                response.write(pug_loggedinmenu({
                    employeecode: request.session.employeecode,
                    name: request.session.username,
                    logintimes: request.session.logintimes,
                ...
                // skriver ut själva editeringsformuläret
    ...
    form.parse(request, function (err, fields) {
        ...
        async function updateQuery() {
            try {
                ...
                // efter att ha sparat skickas användaren tillbaka till listan
...
router.get('/:id', (request, response) => 
{
    ...
    async function sqlQuery() 
    {
        try 
        {
            ...
            // Skriv ut grundläggande Layout & Menyer
            response.write(htmlHead);
            if(request.session && request.session.loggedin) 
            {
                response.write(readHTML('./masterframe/loggedinmenu_css.html'));
            ...
            // skapa den nya Header-designen (Flexbox)
            let htmloutput = "" +
    ...
    try 
    {
        ...
        if (request.session.securityaccesslevel == "A" || request.session.securityaccesslevel == "B") {
            ...
            <div style="display:flex; align-items: center; justify-content: space-between; width: 650px;">
                <a href=""+ config.webaddress+"/api/virusdatabase/editvirus/${str_virusID}" style="color:#336699;text-decoration:none;"> 
        ...
        // REDIRECT-LOGIKEN 
        // skickar användaren vidare efter 2 sekunder
        response.write(`
            <script>
                setTimeout(function() 
                {
